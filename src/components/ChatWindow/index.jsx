...
const ChatWindow = () => {
    ...
    const { messages, sendMessage } = useWebSocket(`https://chat-back.up.railway.app?userID=${clientID}&username=${username}`);
    const [input, setInput] = useState("");


    const handleSendMessage = () => {
        if (input.trim() != "") {
            const date = new Date(Date.now()).toLocaleString('en-US', { timeZone: 'UTC' });
            const data = {
                time: date.split(" ")[1],
                text: input,
                isSent: true,
        ...
        <div className="flex-1 flex flex-col p-4 space-y-4 bg-white dark:bg-gradient-to-b from-blue-700 to-black text-black dark:text-blue-300 border-2  border-black">
            ...
            <div className="flex-1 overflow-y-auto space-y-4">
                {messages.map((msg, index) => (
                    <div
                        key={index}
                        className={`flex ${msg.type == "info" ? "justify-center" : clientID == msg.clientID && msg.isSent ? "justify-end" : "justify-start"
                            } items-end`}
                    >
                        ...
                        )}
                        {
                            msg.type == "info" ?
                                <div
                                    className="p-3 max-w-sm rounded-2xl shadow-md bg-gray-300 dark:bg-gray-800"
                                >
                                    <p>{msg.text}</p>
                                ...
                                :
                                <div
                                    className={`p-3 max-w-sm rounded-2xl shadow-md ${clientID == msg.clientID && msg.isSent
                                ...
                                >
                                    {
                                        ...
                                        <strong className="text-blue-900 text-sm">{msg.username}</strong>
                                    }
                                    <p>{msg.text}</p>
                                    <span className="text-xs text-gray-500 dark:text-blue-300 block text-right">
                                        {msg.time}
            ...
            <div className="p-4 flex items-center space-x-3 bg-gray-200 dark:bg-gray-800 rounded-lg">
                ...
                />
                <button className="px-4 py-2 bg-blue-500 text-white rounded-lg shadow-md hover:bg-blue-600 transition-all" onClick={handleSendMessage}>
                    Send
                </button>
            </div>
        </div>
